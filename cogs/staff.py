...
class Staff(commands.Cog):
    ...
    async def list(self, ctx, req_id=None):
        ...
        for request_id, details in list(data_dict.items()):
            if not req_id or str(request_id) == req_id:
                # all requests.
                found_data += 'request_id: %s\n' % request_id
                found_data += utils.printadict(details)
                found_data += '\n'
        if found_data:
            time.sleep(1)
            pg_data = utils.paginate(found_data)
    ...
    async def found(self, ctx, req_id):
        ...
        if user_id:
            ...
            message += 'Please contact *%s* to welcome this wonderful '
            message += 'villager back to your island.\n'
